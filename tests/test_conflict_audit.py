...
def test_boolean_confidence_is_rejected_by_validation() -> None:
    # float(True) == 1.0 would slip through range checks, but a boolean is
    # not a confidence value; validation now rejects it so the audit and
    # the validator agree on what counts as a confidence.
    issues = validate_facts([fact(fact_id="f001", confidence=True)])

    assert any("invalid confidence True" in issue for issue in issues), issues
    print("PASS: boolean confidence rejected by validation")


def test_cross_agent_takeover_of_disallowed_surface_is_flagged() -> None:
    # agent-b may overwrite, but only on project_scope — relabeling the
    # replacement must not launder a takeover of a user_profile fact.
    warnings = audit_cross_agent_conflicts(AUTHORITIES, [
        fact(fact_id="f001", agent_id="agent-a", surface="user_profile"),
        fact(fact_id="f002", agent_id="agent-b", surface="project_scope",
             confidence=0.9, overwrite_of="f001"),
    ])
